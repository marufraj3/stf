...
class ErpResourceService
{
    private const RESOURCES = [
        ...
        'vehicles' => [
            ...
            'sort' => ['vehicle_name', 'vehicle_number', 'plate_number', 'make', 'status', 'expiry_date', 'registration_date', 'created_at'],
            'presenter' => 'vehicle',
        ],
        'document-types' => [
    ...
    private function saveVehicle(?Model $model, array $data, User $user): Vehicle
    {
        ...
        $validated = Validator::make($data, [
            ...
            'chassisNumber' => ['nullable', 'string', 'max:120'],
            'engineNumber' => ['nullable', 'string', 'max:120'],
        ...
        $vehicle->fill([
            ...
            'issue_date' => $this->nullable($data['issueDate'] ?? null),
            'expiry_date' => $this->nullable($data['expiryDate'] ?? null),
            'renew_date' => $this->nullable($data['renewDate'] ?? null),
            'status' => $validated['status'] ?? 'active',
            'notes' => $data['notes'] ?? null,
